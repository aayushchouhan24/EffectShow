...
function renderUI() {
    const Wrapper = () => {
        const [activeFX, setActiveFX] = React.useState(currentEffects)
        setLevaActiveEffects = setActiveFX

        return <LevaUI 
            activeEffects={activeFX}
            onUploadMusic={audioParams.uploadMusic}
            onBgEffectChange={(val) => {
                bgGuiState.bgEffect = parseInt(val)
                const arr = bgMesh.geometry.attributes.aEffectId.array
                for(let i=0; i<arr.length; i++) arr[i] = bgGuiState.bgEffect
...
function randomizeEffects() {
  ...
  updateShaderMaterial();
  if (setLevaActiveEffects) {
      setLevaActiveEffects([...currentEffects])
  }
}
